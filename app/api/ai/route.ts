...
REGLAS:
1. Devuelve SIEMPRE JSON válido, sin markdown, con las claves summary y html.
...
8. summary debe explicar en una frase qué cambió.`;
...
async function callLLM(messages:any[]){
  ...
  const base={model:process.env.OPEN_SOURCE_LLM_MODEL||'factory-editor',messages,temperature:0.2};
  ...
  // Algunos servidores open source implementan chat-completions pero no response_format.
  ...
  if(typeof content!=='string') throw new Error('Respuesta LLM incompatible. Usa protocolo OpenAI-compatible o adapta app/api/ai/route.ts');
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(!product_id||!String(instruction||'').trim()) throw new Error('Producto e instrucción requeridos');
    ...
    const result=await callLLM([{role:'system',content:SYSTEM},{role:'user',content:`CONTEXTO DEL PRODUCTO:\n${JSON.stringify(context)}\n\nINSTRUCCIÓN:\n${String(instruction).trim()}\n\nHTML ACTUAL:\n${design.html}`}]);
    ...
    const nextVersion=Number(design.current_version||0)+1; const summary=String(result.summary||'Edición generada por IA').slice(0,500);
    const v=await db.from('design_versions').insert({design_id:design.id,version_number:nextVersion,html:result.html,change_summary:summary,source:'link-factory-ai',metadata:{source_app:'link-factory',instruction:String(instruction).slice(0,2000),model:process.env.OPEN_SOURCE_LLM_MODEL||'factory-editor'}}); if(v.error) throw v.error;
    const u=await db.from('design_previews').update({html:result.html,current_version:nextVersion,status:'draft',updated_at:new Date().toISOString(),metadata:{...(design.metadata||{}),last_ai_edit_at:new Date().toISOString()}}).eq('id',design.id); if(u.error) throw u.error;
    const {data:control}=await db.from('controls').select('id').eq('is_root',true).limit(1).maybeSingle();
    if(control?.id) await db.from('event_bus').insert({control_id:control.id,source_provider:'link_factory',event_type:'factory.ai.edit_applied',entity_type:'project',global_id:product_id,dedupe_key:`factory:ai:${product_id}:${nextVersion}:${Date.now()}`,payload:{version:nextVersion,summary,instruction:String(instruction).slice(0,1000)}});
    return NextResponse.json({ok:true,version:nextVersion,summary});
